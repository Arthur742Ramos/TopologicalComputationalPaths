...
| Ramos--de Queiroz--Grisi de Oliveira--de Veras, *Topological Semantics for Scoped Computational Paths* | The parent computational-path setting and its topological semantics | The finite-torus Smith synthesis selected here; the selected bridge fixes a concrete winding-word syntax and realizes it by actual loops |
...
4. `winding_matrix_compatibility`, including the quotient winding classifier,
   ...
   nested concrete winding-word bridge);
...
The concrete winding-word normal-form and matrix-realizability fields are
...
is a fixed `WindingWord` list syntax realized by concatenating actual standard
torus loops; its length, representation, and normal-form laws are derived or
proved for that syntax.  No first-presentation claim is made for the separate
supporting APIs.
