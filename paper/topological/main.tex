...
Completeness alone does not remove this difference: over the harmonic
archipelago, a complete presentation with discrete path labels has a
discrete trace-sensitive based quotient and an indiscrete observable one.
...
Lean checks the quotient comparison, the open-map theorem for locally
path-connected, semilocally simply connected spaces, the universal
fixed-endpoint result, product sorting, finite circle and torus
based completeness and homeomorphisms from their global based-arrow
subspaces to discrete winding groups, and nonconstant circle separation.
...
\item \emph{What information does the topology retain?}
  ...
  A complete presentation over the harmonic archipelago shows why the
  section hypothesis matters.
...
\begin{proposition}[completeness without trace-topology collapse]
\label{prop:complete-trace-separation}
There is a geometrically complete presentation for which the comparison
$J:G_{\mathcal P}^{\mathrm{tr}}\to G_{\mathcal P}^{\mathrm{obs}}$ is not a
homeomorphism.  On the quotient formed from its fixed-endpoint coherent
carrier at one base point, the trace-sensitive topology is discrete and the
observable topology is indiscrete.
\end{proposition}

\begin{proof}
Let $X$ be the harmonic archipelago, with base point $a$.  Take $E$ to be the
set $X^I$ with the discrete topology, and let each step realize the path it
labels.  The endpoint and realization maps are continuous because $E$ is
discrete.  Declare a rewrite between every pair of parallel traces whose
realizations are endpoint-fixed homotopic.  Soundness and the declared rules
make this presentation geometrically complete.

In the fixed-endpoint trace carrier, each trace is an isolated point: this
holds for positive-length words because $E$ is discrete, and for the empty
trace because its endpoint is fixed at $a$.  For a scoped class $C$, its
inverse image in $T_{\mathrm{tr}}(a,a)$ is the union of the full coherent
fibers over traces realizing $C$.  Each such fiber is open in the
trace-sensitive carrier.  Thus every singleton is open in the based
trace-sensitive quotient.

The projection $(p,\gamma)\mapsto\gamma$ from the observable based carrier
to $X^I(a,a)$ is continuous.  It has the continuous section
$\gamma\mapsto(\langle\gamma\rangle,\gamma)$, whose observable code is
$(a,a,1,\gamma,\gamma)$.  It is therefore a quotient map.  Completeness
identifies the scoped relation with endpoint-fixed homotopy, so the based
observable quotient is homeomorphic to $\pi_1^q(X,a)$.  Fabel proved that
this group is uncountable and indiscrete
\cite[Corollaries~9--10]{Fabel05Archipelago}; the
uniform loop topology used there agrees with the compact-open topology for
his metric model.  The based comparison is a continuous bijection from a discrete
space to a non-singleton indiscrete space.

The global comparison fails as well.  Choose a nonidentity class $C$ in
$\pi_1^q(X,a)$.  Its inverse image under the global trace-sensitive quotient
contains no empty traces and is a union of full coherent fibers over
positive-length words, hence is open.  If $\{C\}$ were open in the global
observable quotient, its pullback along the continuous one-letter section
$X^I(a,a)\to T_{\mathrm{obs}}$ would make the homotopy class $C$ open in
$X^I(a,a)$, contrary to the indiscrete quotient topology.
\end{proof}
...
\input{figures/universal-section}
...
The companion Lean proof makes the openness argument explicit. Let $W$ be
open in $X^I$, and choose $\gamma\in W$. For sufficiently small
$\varepsilon>0$, the path
\[
  t\longmapsto \gamma\bigl(\max(0,\min(1,(1+2\varepsilon)t-\varepsilon))\bigr)
\]
still belongs to $W$. It is constant near both endpoints. Choose a finite
compact-open subbasic neighborhood of this reparametrized path inside $W$.
Intersecting the target sets that contain $\gamma(0)$ gives an open set
$U$ around $\gamma(0)$; do the same at $\gamma(1)$ to obtain $V$.
Any paths $\alpha:a'\to\gamma(0)$ and
$\beta:\gamma(1)\to b'$ whose ranges lie in $U$ and $V$ can be inserted
into the two constant tails. The resulting path remains in $W$ and is
homotopic relative to its endpoints to $\alpha\gamma\beta$. The latter
claim follows by a piecewise linear change of speed.

Now suppose $\delta$ represents the same path class as $\gamma$.
The finite null-homotopy ladder, using local path-connectedness and
semilocal simple connectivity, gives a compact-open neighborhood $N$ of
$\delta$. Each $\zeta\in N$ is homotopic to
$\alpha\delta\beta$ for endpoint connectors with ranges in $U$ and $V$.
Replacing $\delta$ by $\gamma$ preserves that homotopy class, and the
absorption construction gives a representative in $W$. Thus $N$ lies in
the saturation of $W$. The saturation is open, so the quotient map $q_I$
is open.
...
\begin{corollary}[totally disconnected spaces]
\label{cor:totally-disconnected-universal}
If $X$ is totally disconnected, $q_I^{\mathcal U}$ is a homeomorphism.
The universal presentation has product-quotient compatibility and
continuous multiplication on the ordinary composable-pair space.
This conclusion does not require local path-connectedness.
\end{corollary}

\begin{proof}
The interval is connected, so every continuous path in $X$ is constant.
Two such paths with the same scoped class have the same source and are
therefore equal. Thus $q_I^{\mathcal U}$ is injective. It is quotient by
Proposition~\ref{prop:universal-path-projection}, hence a homeomorphism.
The remaining claims follow from the resulting openness of
$q_I^{\mathcal U}$ and the open-projection criterion.
\end{proof}
...
These examples show how the ordinary-pullback obstruction behaves in geometric
...
\]
The corresponding interchange for arbitrary factor traces $p:x_0\to x_1$
and $q:y_0\to y_1$ follows from these primitive squares:
\[
...
fixed. To derive the displayed relation, fix a vertical primitive step
$f$ and induct on $p$. The single-step case is the named rectangle rule;
the unit case uses the groupoid unit laws. For a concatenation, paste the
two previously derived squares along their common edge. For an inverse,
reverse a square and cancel the resulting inverse pairs. Induction on $q$
then gives the general trace relation by the same pasting and inverse steps.
Write $\mathcal P\boxtimes\mathcal Q$ for the presentation with primitive
rectangle rules. Its rules are sound: the two paths are the two edge routes from $(x_0,y_0)$
to $(x_1,y_1)$ in the square
$(u,v)\mapsto(\rho(e)(u),\rho(f)(v))$.  The routes are homotopic relative
to their endpoints inside the square.
...
$n$-torus on its based-loop fiber. The product construction includes a
horizontal or vertical copy of each factor step at every point of the other
factor, so its step space is generally infinite. It is distinct from the
finite-generator torus presentation of Theorem~\ref{thm:torus-winding}.
The Lean development checks sorting and based trace completeness for a
presentation that names whole-trace interchange. It also derives every
such rule from primitive squares, proves equivalence of the two scoped
rewrite relations, and transfers completeness to the primitive-only
presentation.
...
Proposition~\ref{prop:complete-trace-separation}, which uses a discrete
topology on the path-label space of the harmonic archipelago, is a
mathematical counterexample and is not part of the Lean selection.
...
the hypotheses of Theorem~\ref{thm:universal-open-groupoid}. The theorem
...
case of Corollary~\ref{cor:totally-disconnected-universal}. Its projection is
...
compatibility. For the universal presentation, the open-map theorem for
locally path-connected, semilocally simply connected spaces is now checked
directly in Lean. It yields an ordinary topological groupoid and discrete
based fibers.
For totally disconnected spaces the path-class projection itself is a
homeomorphism, which gives the same composition conclusion directly.
...
examples.  The complete discrete-label example over the harmonic
archipelago shows that the trace choice cannot be dropped from this
criterion.  Section~\ref{sec:formalization} distinguishes the Lean checks
