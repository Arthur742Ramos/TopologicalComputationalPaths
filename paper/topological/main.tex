...
Lean checks the quotient comparison, conditional open-map transfer,
universal fixed-endpoint result, product sorting, finite circle and torus
...
a homeomorphism, and ordinary composition is continuous. The general
locally path-connected open-map theorem still lacks a Lean proof here.
\LeanName{EndpointVaryingLadder} establishes its local ladder step: every
path has a compact-open neighborhood whose members differ by short endpoint
connectors. \LeanName{EndpointAbsorption} checks that an arbitrary compact-open
neighborhood contains a reparametrized path with positive constant tails.
Insertion of the connectors into those tails and the final open-map proof
...
18-field certificate containing the checked conditional universal and totally
disconnected cases, product results, finite global fibers, and trace-topology
