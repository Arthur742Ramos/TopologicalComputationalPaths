...
# Comparator supplies Landrun options before the command it wants sandboxed.
# Keep that option surface explicit: a dependency update must not silently
# widen the sandbox or consume the command's own arguments as Landrun flags.
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    -unrestricted-*|--unrestricted-*)
      echo "error: refusing Landrun option $1 because it disables sandboxing" >&2
      exit 2
      ;;
