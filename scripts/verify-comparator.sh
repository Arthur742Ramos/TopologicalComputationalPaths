...
case "$config_name" in
  comparator.json|comparator-followup.json|comparator-preimage.json) ;;
  *) echo "usage: $0 [comparator.json|comparator-followup.json|comparator-preimage.json]" >&2; exit 2 ;;
esac
...
# These revisions are the pinned Palomar-compatible toolchain used by the
# current Palomar starter project.  The project Lean toolchain is checked
# against lean4export before any proof is exported.
...
for required_command in cargo git go lake python3; do
  if ! command -v "$required_command" >/dev/null 2>&1; then
    echo "error: $required_command is required for Comparator verification" >&2
    exit 1
  fi
done
